...
%word_stemmed_train = stem_helper(word_dictionary.mat)
%word_stemmed_test = stem_helper(word_test)
...
%     price_city_lib = liblinear_train(rand(size(score_test_city,1),1), score_test_sparse, ['-s 6']);
     ...
     svm_mod = svmtrain(Y_train, X_train, '-s 3 -c 60');
...
rmse_60 = sqrt(mean(abs(Y_hat - price_test).^2));
toc
